...
define("PACS_PATH", "c:", TRUE);
define("IMG_SIZE_1", 512, TRUE);
define("IMG_SIZE_2", 256, TRUE);

$hospital=$_SESSION['hospital'];
mysql_connect(EW_CONN_HOST.":".EW_CONN_PORT ,EW_CONN_USER, EW_CONN_PASS);
mysql_select_db("pacs_00");
$result=mysql_query("select * from hospital WHERE sn=".$hospital." ");
while($row=mysql_fetch_array($result)){
	define("EW_CONN_DB", $row[3], TRUE);
	$hname=$row[1];
}
...
define("EW_PROJECT_NAME", "ps01", TRUE); // Project Name
define("EW_RANDOM_KEY", 'De047zLDyPFdL4Zr', TRUE); // Random key for encryption

...
// Define page object
$patient_detail_edit = new cpatient_detail_edit();
$Page =& $patient_detail_edit;

// Page init processing
$patient_detail_edit->Page_Init();

// Page main processing
$patient_detail_edit->Page_Main();
?>
...
// Create page object
var patient_detail_edit = new ew_Page("patient_detail_edit");

// page properties
patient_detail_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = patient_detail_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
patient_detail_edit.ValidateForm = function(fobj) {
...
patient_detail_edit.Form_CustomValidate = 
...
patient_detail_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
patient_detail_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker">Edit TABLE: Patient Detail<br><br>
<a href="<?php echo $patient_detail->getReturnUrl() ?>">Go Back</a></span></p>
<?php $patient_detail_edit->ShowMessage() ?>
<form name="fpatient_detailedit" id="fpatient_detailedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return patient_detail_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="patient_detail">
...
<?php if ($patient_detail->DetailNo->Visible) { // DetailNo ?>
	<tr<?php echo $patient_detail->DetailNo->RowAttributes ?>>
		...
		<td<?php echo $patient_detail->DetailNo->CellAttributes() ?>><span id="el_DetailNo">
<div<?php echo $patient_detail->DetailNo->ViewAttributes() ?>><?php echo $patient_detail->DetailNo->EditValue ?></div><input type="hidden" name="x_DetailNo" id="x_DetailNo" value="<?php echo ew_HtmlEncode($patient_detail->DetailNo->CurrentValue) ?>">
</span><?php echo $patient_detail->DetailNo->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cpatient_detail_edit {
	...
	// Page Object Name
	var $PageObjName = 'patient_detail_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$patient_detail->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$patient_detail->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$patient_detail->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($patient_detail->DetailNo->CurrentValue == "")
			$this->Page_Terminate("patient_detail_list.php"); // Invalid key, return to list
		switch ($patient_detail->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$patient_detail->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$patient_detail->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($patient_detail->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// DetailNo
			$patient_detail->DetailNo->EditCustomAttributes = "";
			$patient_detail->DetailNo->EditValue = $patient_detail->DetailNo->CurrentValue;
			$patient_detail->DetailNo->CssStyle = "";
			$patient_detail->DetailNo->CssClass = "";
			$patient_detail->DetailNo->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $patient_detail->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($patient_detail->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$patient_detail->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}

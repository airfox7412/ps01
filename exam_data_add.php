...
// Define page object
$exam_data_add = new cexam_data_add();
$Page =& $exam_data_add;

// Page init processing
$exam_data_add->Page_Init();

// Page main processing
$exam_data_add->Page_Main();
?>
...
// Create page object
var exam_data_add = new ew_Page("exam_data_add");

// page properties
exam_data_add.PageID = "add"; // page ID
var EW_PAGE_ID = exam_data_add.PageID; // for backward compatibility

// extend page with ValidateForm function
exam_data_add.ValidateForm = function(fobj) {
...
exam_data_add.Form_CustomValidate = 
...
exam_data_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
exam_data_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker">Add to TABLE: Exam Data<br><br>
<a href="<?php echo $exam_data->getReturnUrl() ?>">Go Back</a></span></p>
<?php $exam_data_add->ShowMessage() ?>
<form name="fexam_dataadd" id="fexam_dataadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return exam_data_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_ExamDataNo" id="x_ExamDataNo" size="30" value="<?php echo $exam_data->ExamDataNo->EditValue ?>"<?php echo $exam_data->ExamDataNo->EditAttributes() ?>>
...
<input type="file" name="x_Soap" id="x_Soap" size="30"<?php echo $exam_data->Soap->EditAttributes() ?>>
</div>
...
class cexam_data_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $exam_data->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$exam_data->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $exam_data->CurrentAction = "C"; // Copy Record
		  } else {
		    $exam_data->CurrentAction = "I"; // Display Blank Record
		...
		switch ($exam_data->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$exam_data->SendEmail = TRUE; // Send email on add success
		...
		$exam_data->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $exam_data;
		$exam_data->ExamDataNo->CurrentValue = 0;
		$exam_data->PatientMainNo->CurrentValue = 0;
		$exam_data->InstitutionNo->CurrentValue = 0;
		$exam_data->PatientSubKindNo->CurrentValue = 0;
		$exam_data->SpecialExamDate->CurrentValue = "0000-00-00 00:00:00";
		$exam_data->ExamDate->CurrentValue = "0000-00-00";
		$exam_data->ExamTime->CurrentValue = "00:00:00";
		$exam_data->LogDate->CurrentValue = "0000-00-00 00:00:00";
		$exam_data->Status->CurrentValue = "N";
	}
	...
	function RenderRow() {
		...
		} elseif ($exam_data->RowType == EW_ROWTYPE_ADD) { // Add row

			// ExamDataNo
			$exam_data->ExamDataNo->EditCustomAttributes = "";
			$exam_data->ExamDataNo->EditValue = ew_HtmlEncode($exam_data->ExamDataNo->CurrentValue);
